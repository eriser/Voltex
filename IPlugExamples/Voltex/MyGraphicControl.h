...
public:
	MyGraphicControl(IPlugBase *pPlug, IRECT pR) : IControl(pPlug, pR) {
		Point newPoint1;
		newPoint1.x = convertToPercentX(275);
		newPoint1.y = convertToPercentY(500);
		newPoint1.uid = counter++;
		...
		Point newPoint2;
		newPoint2.x = convertToPercentX(952);
		newPoint2.y = convertToPercentY(500);
		newPoint2.uid = counter++;
